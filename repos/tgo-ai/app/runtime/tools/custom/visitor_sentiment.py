"""Team-level tool to track/update visitor sentiment/state information."""
...
def create_visitor_sentiment_tool(
    *,
    team_id: str,
    session_id: str | None,
    user_id: str | None,
    ...
    """Create a team-level tool that updates visitor sentiment/state."""
    ctx = ToolContext(team_id, session_id, user_id, project_id)
    client = EventClient(ctx)

    error_messages = {
        "not_configured": "抱歉，当前无法为您更新访客状态，我们已记录该问题并会尽快处理。请稍后再试或直接联系客服。",
        "api_error": "抱歉，访客状态更新未能成功提交。请稍后重试或联系技术支持。",
        "http_error": "抱歉，网络异常导致访客状态更新未能提交。请稍后重试或联系技术支持。",
        "unexpected_error": "抱歉，出现异常，暂时无法为您更新访客状态。请稍后重试或联系技术支持。",
    }

    async def update_visitor_sentiment(
        *,
        ...
        """Update visitor sentiment/state via visitor_sentiment.update event."""
        ...
        if not provided:
            return "请至少提供一个需要更新的访客状态字段，例如满意度、情绪或意图。"

        result = await client.post_event(
            "visitor_sentiment.update",
        ...
        return f"已记录访客状态更新：{updated_keys}。"

    return Function(
        name="update_visitor_sentiment",
        description=(
            "当你在对话中识别到访客满意度、情绪或意图发生变化时，调用此工具以记录/更新访客状态。"
        ...
        entrypoint=update_visitor_sentiment,
